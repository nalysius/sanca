...
/// The checker
pub struct EmailSubscribersChecker<'a> {
...
impl<'a> EmailSubscribersChecker<'a> {
    /// Creates the checker.
    ...
    fn check_http_body(&self, url_response: &UrlResponse) -> Option<Finding> {
        trace!(
            "Running EmailSubscribersChecker::check_http_body() on {}",
            url_response.url
        );

        if url_response
            .url
            .contains("/wp-content/plugins/email-subscribers/readme.txt")
        {
            let body_regex_params = self
                .regexes
                .get("http-body-source")
                .expect("Regex EmailSubscribers/http-body-source not found");
            let (regex, keep_left, keep_right) = body_regex_params;
            let caps_result = regex.captures(&url_response.body);

            // The regex matches
            if caps_result.is_some() {
                info!("Regex EmailSubscribers/http-body-source matches");
...
impl<'a> Checker for EmailSubscribersChecker<'a> {}

impl<'a> HttpChecker for EmailSubscribersChecker<'a> {
    /// Check for a HTTP scan.
    fn check_http(&self, data: &[UrlResponse]) -> Vec<Finding> {
        trace!("Running EmailSubscribersChecker::check_http()");
...
mod tests {
    ...
    fn source_code_matches() {
        let checker = EmailSubscribersChecker::new();
        let body1 = r#"=== Email Subscribers - The Most Popular Email Plugin ===
        ...
        Stable tag: 5.3.22
        Requires PHP: 7.2"#;
        let url1 = "https://www.example.com/blog/wp-content/plugins/email-subscribers/readme.txt";
    ...
    fn source_code_doesnt_match() {
        let checker = EmailSubscribersChecker::new();
        let body = r#"EmailSubscribers can be found in stable tag: 5.10.30"#;
    ...
    fn finds_match_in_url_responses() {
        let checker = EmailSubscribersChecker::new();
        let body1 = r#"=== WP Email Subscribers - The Email Plugin ===
        Contributors: wpforms, jaredatch, smub, slaFFik, capuderg
        Tags: smtp, wp mail smtp, wordpress smtp, gmail smtp, sendgrid smtp, mailgun smtp, mail, mailer, phpmailer, wp_mail, email, mailgun, sengrid, gmail, sendinblue, brevo, wp smtp
        Requires at least: 5.2
        Tested up to: 6.4
        Stable tag: 5.9.9
        Requires PHP: 7.2"#;
        let url1 = "https://www.example.com/wp-content/plugins/email-subscribers/readme.txt";
        let url_response_valid =
            UrlResponse::new(url1, HashMap::new(), body1, UrlRequestType::Default, 200);
        let url_response_invalid = UrlResponse::new(
    ...
    fn doesnt_find_match_in_url_responses() {
        ...
        let body2 = r#"How to install EmailSubscribers 2.2.4"#;
