...
/// The checker
pub struct LiteSpeedCacheChecker<'a> {
...
impl<'a> LiteSpeedCacheChecker<'a> {
    /// Creates the checker.
    ...
    fn check_http_body(&self, url_response: &UrlResponse) -> Option<Finding> {
        trace!(
            "Running LiteSpeedCacheChecker::check_http_body() on {}",
            url_response.url
        );

        if url_response
            .url
            .contains("/wp-content/plugins/litespeed-cache/readme.txt")
        {
            let body_regex_params = self
                .regexes
                .get("http-body-source")
                .expect("Regex LiteSpeedCache/http-body-source not found");
            let (regex, keep_left, keep_right) = body_regex_params;
            let caps_result = regex.captures(&url_response.body);

            // The regex matches
            if caps_result.is_some() {
                info!("Regex LiteSpeedCache/http-body-source matches");
...
impl<'a> HttpChecker for LiteSpeedCacheChecker<'a> {
    ...
    fn check_http(&self, data: &[UrlResponse]) -> Vec<Finding> {
        trace!("Running LiteSpeedCacheChecker::check_http()");
...
mod tests {
    ...
    fn source_code_matches() {
        let checker = LiteSpeedCacheChecker::new();
        let body1 = r#"=== LiteSpeed Cache ===
        Contributors: LiteSpeedTech
        Tags: caching, optimize, performance, pagespeed, core web vitals, seo, speed
        Requires at least: 4.0
        Tested up to: 6.3.2
        Stable tag: 5.7.0.1"#;
        let url1 = "https://www.example.com/blog/wp-content/plugins/litespeed-cache/readme.txt";
    ...
    fn source_code_doesnt_match() {
        let checker = LiteSpeedCacheChecker::new();
        let body = r#"LiteSpeedCache can be found in stable tag: 5.0.7.2"#;
    ...
    fn finds_match_in_url_responses() {
        ...
        Tested up to: 6.3.2
        Stable tag: 5.6.2.8"#;
        let url1 = "https://www.example.com/wp-content/plugins/litespeed-cache/readme.txt";
        let url_response_valid =
            UrlResponse::new(url1, HashMap::new(), body1, UrlRequestType::Default, 200);
        let url_response_invalid = UrlResponse::new(
    ...
    fn doesnt_find_match_in_url_responses() {
        ...
        let body2 = r#"How to install LiteSpeedCache 4.2.4"#;
