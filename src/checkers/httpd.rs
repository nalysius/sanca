...
impl<'a> ApacheHttpdChecker<'a> {
    ...
    fn check_http_headers(&self, url_response: &UrlResponse) -> Option<Finding> {
        ...
        for (header_name, header_value) in headers_to_check {
            ...
            if caps_result.is_some() {
                ...
                return Some(self.extract_finding_from_captures(caps, Some(url_response), 45, 45, "Apache httpd", &format!("$techno_name$$techno_version$ has been identified using the HTTP header \"{}: $evidence$\" returned at the following URL: $url_of_finding$", header_name)));
            }
        }
        None
    }
    ...
    fn check_http_body(&self, url_response: &UrlResponse) -> Option<Finding> {
        ...
        if caps_result.is_some() {
            ...
            let caps = caps_result.unwrap();
            return Some(self.extract_finding_from_captures(caps, Some(url_response), 45, 45, "Apache httpd", "$techno_name$$techno_version$ has been identified by looking at its signature \"$evidence$\" at this page: $url_of_finding$"));
        }
        None
    }
}
