...
/// The TwistedWeb checker
pub struct TwistedWebChecker<'a> {
...
impl<'a> TwistedWebChecker<'a> {
    ...
    pub fn new() -> Self {
        ...
        let header_regex =
            Regex::new(r"(?P<wholematch>.*TwistedWeb\/(?P<version1>\d+\.\d+\.\d+).*)").unwrap();
    ...
    fn check_http_headers(&self, url_response: &UrlResponse) -> Option<Finding> {
        trace!(
            "Running TwistedWebChecker::check_http_headers() on {}",
            url_response.url
        );
        ...
        let body_regex_params = self
            .regexes
            .get("http-header")
            .expect("Regex TwistedWeb/http-header not found");
        ...
        for (header_name, header_value) in headers_to_check {
            ...
            let caps_result = regex.captures(&header_value);

            // The regex matches
            if caps_result.is_some() {
                info!("Regex TwistedWeb/http-header matches");
...
impl<'a> Checker for TwistedWebChecker<'a> {}

impl<'a> HttpChecker for TwistedWebChecker<'a> {
    /// Check if the asset is running TwistedWeb.
    ...
    fn check_http(&self, data: &[UrlResponse]) -> Vec<Finding> {
        trace!("Running TwistedWebChecker::check_http()");
...
mod tests {
    ...
    fn header_matches() {
        ...
        headers1.insert("Server".to_string(), "TwistedWeb/8.2.1".to_string());
    ...
    fn finds_match_in_url_responses() {
        ...
        headers1.insert(
            "Server".to_string(),
            "nginx/1.22.2 OpenSSL/1.0.2k TwistedWeb/8.1.1".to_string(),
        );
    ...
    fn doesnt_find_match_in_url_responses() {
        ...
        assert!(
            findings.is_empty(),
            "TwistedWeb must not be detected on JavaScript URLs to avoid false positive"
        );
    }
}
